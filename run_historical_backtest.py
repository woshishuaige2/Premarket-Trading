...
def main():
    ...
    print(f"╔══════════════════════════════════════════════════════════════╗")
    print(f"║ PREMARKET STRATEGY HISTORICAL BACKTEST                       ║")
    print(f"╠══════════════════════════════════════════════════════════════╣")
    print(f"║ DATE: {args.date:<54} ║")
    print(f"║ SYMBOLS: {', '.join(symbols):<51} ║")
    print(f"╚══════════════════════════════════════════════════════════════╝")
    ...
    try:
        for symbol in symbols:
            ...
            if not trades:
                ...
                print(f"\n[TRADES] {symbol}:")
                print(f"{'ENTRY TIME':<20} | {'EXIT TIME':<20} | {'ENTRY':<8} | {'EXIT':<8} | {'PNL':<8} | {'REASON'}")
                print("-" * 85)
                for t in trades:
                    e_time = t['entry_time'].strftime("%H:%M:%S")
                    x_time = t['exit_time'].strftime("%H:%M:%S")
                    print(f"{e_time:<20} | {x_time:<20} | {t['entry_price']:<8.2f} | {t['exit_price']:<8.2f} | {t['pnl']:<8.2f} | {t['reason']}")
                
                total_pnl = sum(t['pnl'] for t in trades)
                print(f"\n[SUMMARY] {symbol} Total PnL: ${total_pnl:.2f} | Final Capital: ${final_capital:.2f}\n")
